...
import ZhimaContractDialog from "../../components/zhima-contract-dialog";
...
const OrderPage = () => {
  ...
  const [showZhima, setShowZhima] = useState(false);
  ...
  return (
    <div className={styles["container"]}>
      ...
      <ZhimaContractDialog
        open={showZhima}
        courseId={Number(goods.id)}
        onCancel={() => setShowZhima(false)}
        onAgree={() => {
          setShowZhima(false);
          Toast.show("合同已确认，先学后付扣款将在支付宝芝麻信用联调后开通");
        }}
      />
    </div>
  );
