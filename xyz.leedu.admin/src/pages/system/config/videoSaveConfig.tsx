...
const SystemVideoSaveConfigPage = () => {
    ...
    system
      ...
      .then((res: any) => {
        ...
        for (let index in configData) {
          if (configData[index].key === "leedu.upload.video.aliyun.region") {
            form.setFieldsValue({
              "leedu.upload.video.aliyun.region": configData[index].value,
            });
          } else if (
            configData[index].key === "leedu.upload.video.aliyun.host"
          ) {
            form.setFieldsValue({
              "leedu.upload.video.aliyun.host": configData[index].value,
            });
          } else if (
            configData[index].key === "leedu.upload.video.aliyun.access_key_id"
          ) {
            form.setFieldsValue({
              "leedu.upload.video.aliyun.access_key_id":
                configData[index].value,
            });
          } else if (
            configData[index].key ===
            "leedu.upload.video.aliyun.access_key_secret"
          ) {
            form.setFieldsValue({
              "leedu.upload.video.aliyun.access_key_secret":
                configData[index].value,
            ...
            configData[index].key === "leedu.upload.video.aliyun.play_domain"
          ) {
            form.setFieldsValue({
              "leedu.upload.video.aliyun.play_domain": configData[index].value,
            });
            ...
            configData[index].key === "leedu.upload.video.default_service"
          ) {
            form.setFieldsValue({
              "leedu.upload.video.default_service": configData[index].value,
            });
            let arr: any = [];
  ...
  const onFinish = (values: any) => {
    ...
    let it: any = aliRegions.find(
      (o: any) => o.value === values["leedu.upload.video.aliyun.region"]
    );
    if (it) {
      values["leedu.upload.video.aliyun.host"] = it.host;
    }
    setLoading(true);
    system
      .saveSetting({
      ...
      {!loading && (
        <div className="float-left">
          ...
          >
            ...
            <Form.Item
              label="视频存储默认服务"
              name="leedu.upload.video.default_service"
            >
              <Select style={{ width: 300 }} allowClear options={selects} />
            </Form.Item>
            <div className="from-title mt-30">阿里云视频</div>
            <Form.Item label="Region" name="leedu.upload.video.aliyun.region">
              <Select style={{ width: 300 }} allowClear options={aliRegions} />
            </Form.Item>
            <Form.Item
              label="AccessKeyId"
              name="leedu.upload.video.aliyun.access_key_id"
            >
              <Input style={{ width: 300 }} allowClear />
            </Form.Item>
            <Form.Item
              label="AccessKeySecret"
              name="leedu.upload.video.aliyun.access_key_secret"
            >
              <Input style={{ width: 300 }} allowClear />
            </Form.Item>
            <Form.Item
              label="播放域名"
              name="leedu.upload.video.aliyun.play_domain"
            >
              <Input style={{ width: 300 }} allowClear />
            </Form.Item>
