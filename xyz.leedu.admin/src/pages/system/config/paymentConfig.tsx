...
const SystemPaymentConfigPage = () => {
    ...
    system
      ...
      .then((res: any) => {
        ...
        for (let index in configData) {
          if (configData[index].key === "leedu.payment.alipay.enabled") {
            form.setFieldsValue({
              "leedu.payment.alipay.enabled": Number(configData[index].value),
            });
          ...
          } else if (configData[index].key === "leedu.payment.wechat.enabled") {
            form.setFieldsValue({
              "leedu.payment.wechat.enabled": Number(configData[index].value),
            });
            ...
            configData[index].key === "leedu.payment.handPay.enabled"
          ) {
            form.setFieldsValue({
              "leedu.payment.handPay.enabled": Number(configData[index].value),
            });
          } else if (
            configData[index].key === "leedu.payment.handPay.introduction"
          ) {
            form.setFieldsValue({
              "leedu.payment.handPay.introduction": configData[index].value,
            });
  ...
  const aliPayChange = (checked: boolean) => {
    if (checked) {
      form.setFieldsValue({ "leedu.payment.alipay.enabled": 1 });
    } else {
      form.setFieldsValue({ "leedu.payment.alipay.enabled": 0 });
    }
  };

  const wechatChange = (checked: boolean) => {
    if (checked) {
      form.setFieldsValue({ "leedu.payment.wechat.enabled": 1 });
    } else {
      form.setFieldsValue({ "leedu.payment.wechat.enabled": 0 });
    }
  };

  const handPayChange = (checked: boolean) => {
    if (checked) {
      form.setFieldsValue({ "leedu.payment.handPay.enabled": 1 });
    } else {
      form.setFieldsValue({ "leedu.payment.handPay.enabled": 0 });
    }
  };
      ...
      {!loading && (
        <div className="float-left">
          ...
          >
            ...
            <Form.Item
              label="支付宝支付"
              name="leedu.payment.alipay.enabled"
            ...
            <Form.Item
              label="微信支付"
              name="leedu.payment.wechat.enabled"
              valuePropName="checked"
            >
            ...
            <Form.Item
              label="手动打款"
              name="leedu.payment.handPay.enabled"
              valuePropName="checked"
            >
            ...
            <Form.Item
              label="手动打款说明"
              name="leedu.payment.handPay.introduction"
              style={{ height: 440 }}
            >
              <div className="w-800px">
                <QuillEditor
                  ...
                  setContent={(value: string) => {
                    form.setFieldsValue({
                      "leedu.payment.handPay.introduction": value,
                    });
                  }}
                ></QuillEditor>
              </div>
