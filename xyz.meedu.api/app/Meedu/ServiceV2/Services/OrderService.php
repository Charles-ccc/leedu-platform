...
class OrderService implements OrderServiceInterface
{
    ...
    public function createOrder(int $userId, $orderGoodsInfo, $promoCode): array
    {
        return DB::transaction(function () use ($userId, $orderGoodsInfo, $promoCode) {
            ...
            // 平台化:解析订单所属机构 + 下单时绑定的业务员
            $commissionService = app()->make(\App\Meedu\Merchant\CommissionService::class);
            $merchantId = $commissionService->resolveMerchantIdFromGoods($orderGoodsInfo['type'], (int)$orderGoodsInfo['id']);
            $boundSalespersonId = $commissionService->resolveActiveSalesperson($userId);
            ...
            $orderData = [
                ...
                'bound_salesperson_id' => $boundSalespersonId,
            ];
            ...
            // 平台化:下单即计提业务员提成(机构承担)
            $commissionService->accrue($merchantId, $boundSalespersonId, (int)$order['id'], (int)$total);
