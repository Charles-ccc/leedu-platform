...
use App\Meedu\Merchant\CommissionService;
...
class LoginController extends BaseController
{
     ...
     * @apiSuccess {String} data.token token
     */
    public function mobileLogin(MobileLoginRequest $request, CommissionService $commissionService)
    {
        $this->mobileCodeCheck();

        ['mobile' => $mobile, 'promo_code' => $promoCode] = $request->filldata();

        $user = $this->userService->findMobile($mobile);
        if (!$user) {
            // 新用户(短信登录即自动注册):必须填写有效的业务员推广码
            $salesperson = $commissionService->findActiveByInviteCode((string)$promoCode);
            if (!$salesperson) {
                return $this->error(__('新用户注册请输入有效的业务员推广码'));
            }
