...
> 把单商户网校（Leedu 开源版）改造为**多机构入驻平台（淘宝模式）+ 平台级业务员拓客返点**体系。
...
**`agreement_templates`（协议模板，按类型+版本管理）** — 沿用 leedu `AgreementService` 思路
```
...
> 全部用**增量 migration** 新增，不改历史 migration，保证可回滚、不破坏 `leedu:upgrade`。
...
9. **升级兼容**：全部增量 migration，不破坏 `leedu:upgrade`。
