...
Leedu 的视觉系统不是一份品牌宣言，是一张**工作台桌面**。运营人员每天要在这张桌面上完成订单核销、课程上下架、学员管理、内容审核与异常排查；学员要在 pc/h5 端不被打断地完成"找课 → 购买 → 学习"。系统的工作是让这两种心流都不掉线——而不是让人停下来欣赏 UI。
...
**Character:** 单一无衬线家族贯穿全产品——Inter 在西文上几何感强、字距可控；系统字体兜底确保中文环境下使用 PingFang / Microsoft YaHei 的原生字形。不引入展示字体，不用宋体作 display——Leedu 不卖"文化感"，卖"靠谱感"。
...
- **Lifted** (`box-shadow: 20px 20px 100px 0 rgba(85, 102, 119, 0.1)`): 登录卡、初始化向导主面板使用的**长柔阴影**。偏移大、模糊大、不透明度低——产生"漂浮在工作台上方"的感觉，不产生厚重感。这是 Leedu 的签名阴影。
...
表格中的"状态列"是 Leedu admin 的核心场景。规范：
