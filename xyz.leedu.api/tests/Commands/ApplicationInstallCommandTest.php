...
class ApplicationInstallCommandTest extends OriginalTestCase
{
    ...
    public function test_install_administrator()
    {
        $this->artisan('install', ['action' => 'administrator'])
            ->expectsOutput('请先运行 [ php artisan install role ] 命令来初始化leedu的管理员权限数据')
            ->assertFailed();
    }
    ...
    public function test_install_administrator_q()
    {
        ...
        $admin = Administrator::query()->where('email', 'leedu@leedu.leedu')->first();
    ...
    public function test_install_administrator_with_empty_email()
    {
        ...
        $this->artisan('install', ['action' => 'administrator'])
            ->expectsQuestion('请输入邮箱(默认：leedu@leedu.leedu):', '')
            ->expectsOutput('邮箱不能空')
            ->assertFailed();
    }
    ...
    public function test_install_administrator_with_role()
    {
        ...
        $this->artisan('install', ['action' => 'administrator'])
            ->expectsQuestion('请输入邮箱(默认：leedu@leedu.leedu):', $email)
            ->expectsQuestion('请输入密码(默认：leedu123):', $password)
            ->expectsQuestion('请再输入一次(默认：leedu123):', $password)
            ->expectsOutput('管理员初始化成功')
    ...
    public function test_install_administrator_and_password_not_correct()
    {
        ...
        $this->artisan('install', ['action' => 'administrator'])
            ->expectsQuestion('请输入邮箱(默认：leedu@leedu.leedu):', $email)
            ->expectsQuestion('请输入密码(默认：leedu123):', $password)
            ->expectsQuestion('请再输入一次(默认：leedu123):', Str::random())
    ...
    public function test_install_role()
    {
        ...
        $role = AdministratorRole::query()->where('slug', config('leedu.administrator.super_slug'))->first();
