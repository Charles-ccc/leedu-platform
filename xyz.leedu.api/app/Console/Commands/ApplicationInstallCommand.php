...
class ApplicationInstallCommand extends Command
{
    ...
    protected $description = 'Leedu程序安装命令';
    ...
    {
        $super = AdministratorRole::query()->where('slug', config('leedu.administrator.super_slug'))->first();
        if (!$super) {
            $this->warn('请先运行 [ php artisan install role ] 命令来初始化leedu的管理员权限数据');
        ...
        if (!$this->option('q')) {
            $name = '超级管理员';
            $email = $this->ask('请输入邮箱(默认：leedu@leedu.leedu):', 'leedu@leedu.leedu');
            if (!$email) {
                $this->warn('邮箱不能空');
            ...
            $password = '';
            while ($password === '') {
                $password = $this->ask('请输入密码(默认：leedu123):', 'leedu123');
            }

            $passwordRepeat = '';
            while ($passwordRepeat === '') {
                $passwordRepeat = $this->ask('请再输入一次(默认：leedu123):', 'leedu123');
            }
