...
use App\Leedu\Schedule\ScheduleContainer;
...
class Kernel extends ConsoleKernel
{
    ...
    protected $bootstrappers = [
        ...
        \App\Leedu\AddonsProvider::class,
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        // 订单退款查询处理
        $schedule->command('leedu:refund:query')
            ->onOneServer()
            ->everyFiveMinutes()
            ->appendOutputTo(storage_path('logs/order_refund.log'));

        // 用户注销任务
        $schedule->command('leedu:user-delete-job')
            ->onOneServer()
            ->everyThirtyMinutes()
            ->appendOutputTo(storage_path('logs/user-delete-job.log'));

        // 全文搜索课程索引任务=>定时同步课程上架时间已到的课程
        $schedule->command('leedu:full-search:published-courses-index')
            ->onOneServer()
            ->everyMinute()
