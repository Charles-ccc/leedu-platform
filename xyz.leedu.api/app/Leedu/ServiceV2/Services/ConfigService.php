...
class ConfigService implements ConfigServiceInterface
{
    ...
    {
        return config('leedu.administrator.super_slug') ?? '';
    }

    public function getEnabledSocialiteApps(): array
    {
        return collect(config('leedu.member.socialite', []))->filter(function ($item) {
    ...
    {
        return (int)config('leedu.system.login.limit.rule', 0);
    }

    public function isCloseDeprecatedApi(): bool
    {
        return (bool)config('leedu.system.close_deprecated_api');
    }
    ...
    {
        return (int)config('leedu.member.enabled_face_verify') === 1;
    }

    public function getVideoDefaultService(): string
    {
        return config('leedu.upload.video.default_service') ?? '';
    }
    ...
    {
        return rtrim(config('leedu.system.pc_url') ?? '');
    }

    public function getH5Url(): string
    {
        return rtrim(config('leedu.system.h5_url') ?? '');
    }

    public function getLogo(): string
    {
        $url = config('leedu.system.logo') ?? '';
        if ($url) {
            $url = url($url);
        }
        return $url;
    ...
    {
        return config('leedu.member.vip_protocol') ?? '';
    }

    public function getAliVodCallbackKey(): string
    {
        return config('leedu.upload.video.aliyun.callback_key') ?? '';
    }
    ...
    {
        return (int)config('leedu.payment.wechat.enabled') === 1;
    }

    public function enabledAlipayPayment(): bool
    {
        return (int)config('leedu.payment.alipay.enabled') === 1;
    }

    public function enabledHandPayPayment(): bool
    {
        return (int)config('leedu.payment.handPay.enabled') === 1;
    }
    ...
    {
        return url(config('leedu.member.default_avatar'));
    }

    public function getWechatMpConfig(): array
    {
        return config('leedu.mp_wechat');
    }
    ...
    {
        return array_column(config('leedu.hook.comment.rt'), 'id');
    }
}
