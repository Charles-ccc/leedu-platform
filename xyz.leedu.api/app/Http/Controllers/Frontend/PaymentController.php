...
use App\Leedu\Cache\MemoryCache;
...
class PaymentController extends FrontendController
{
    ...
    public function callback(ConfigServiceInterface $configService, OrderServiceInterface $orderService, $payment)
    {
        if (BusConstant::PAYMENT_ALIPAY === $payment) {
            // 平台化:按订单所属机构解析配置来验签(回调先取 out_trade_no 找订单拿 merchant_id)
            $preOutTradeNo = request()->input('out_trade_no');
            $preOrder = $preOutTradeNo ? $orderService->findByOrderNo($preOutTradeNo) : null;
            $merchantId = (int)($preOrder['merchant_id'] ?? 0);
            $alipayConfig = app(\App\Leedu\Merchant\AlipayClientResolver::class)->configForMerchant($merchantId);
            $pay = Pay::alipay($alipayConfig);
            $data = $pay->verify();
