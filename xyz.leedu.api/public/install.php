...
<title>{$titleSafe} · Leedu 安装</title>
<style>
:root {
...
<div class="page">
    <div class="card">
        <div class="brand">
            <a href="https://leedu.vip/" target="_blank" rel="noopener"><img src="/images/logo.png" alt="Leedu"></a>
        </div>
...
HTML;
    shell_html($title, 'Leedu 安装', $body);
    exit;
}
...
if (mb_substr($uri, 0, 7) === '/public') {
    alert_page(
        '根目录配置错误',
        '当前请求路径以 /public 开头，说明 Web 服务的根目录指向了 Leedu 程序根目录，而不是其下的 public 目录。',
        '请将 Nginx / Apache 的站点根目录改为 Leedu 程序根目录下的 public 子目录后再访问本页。'
    );
}
...
if ($step === 0) {
    ...
    <p class="subtitle">检查当前服务器是否满足 Leedu 的运行要求。通过后即可继续配置数据库。</p>
    ...
    <form id="dbForm" action="" method="POST" class="form" autocomplete="off">
        ...
        <div class="field">
            ...
            <input class="field-input" type="text" id="config-url" name="url" value="<?php echo $h($url); ?>" placeholder="例如 https://leedu.example.com" required>
        ...
        <div class="field">
            ...
            <input class="field-input" type="text" id="dbName" name="db_db" value="<?php echo $h($dbDb); ?>" placeholder="leedu" required>
    ...
    <h1 class="title">Leedu 已部署到这台服务器</h1>
